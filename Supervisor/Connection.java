...
public class Connection {
	...
	public void sendRequest() {
		try {
			...
			jsonObject.put("isSupervisor", true);
	...
	public void sendAttendRequest() {
	    try {
	        ...
	        // Create a JSON object for the attend request
	        JSONObject attendRequest = new JSONObject();
	        attendRequest.put("clientId", clientId);  // Use the current client ID
	        attendRequest.put("name", name);
	        attendRequest.put("isSupervisor", true);  // Indicate this is a supervisor action
	        attendRequest.put("attend", true);        // The action is to attend to the first student
	        attendRequest.put("message", gui.getMessage());

	        // Send the JSON object as a string
	        String requestMessage = attendRequest.toString();
	        requestSocket.send(requestMessage);
	        System.out.println("Attend request sent: " + requestMessage);

	        // Wait for the server's reply
	        byte[] reply = requestSocket.recv();
	        String replyStr = new String(reply, ZMQ.CHARSET);
	        System.out.println("Received reply for attend request: " + replyStr);
	...
	// Method to subscribe to a topic using SUB socket
	public void RunnablesubscribeToQueue() {
		try {
			...
			if (subscribeSocket == null) {
	            ...
	            subscribeSocket.subscribe("<name of user>".getBytes(ZMQ.CHARSET)); // Subscribe to the "<name of user>" topic
	        ...
	        while (!Thread.currentThread().isInterrupted()) {
	            ...
	            } else if (topic.equals("<name of user>")) {
	                synchronized (this) {
	                	String fakeMessage = "Hello";
	                    receivedUserMsg = receivedMsg; // Store the message in receivedUserMsg
